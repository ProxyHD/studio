...
const aiImage = PlaceHolderImages.find(img => img.id === 'news-ai-assistant');
const wellbeingImage = PlaceHolderImages.find(img => img.id === 'news-wellbeing');
const themesImage = PlaceHolderImages.find(img => img.id === 'news-themes');
...
const newsData: Record<Locale, NewsItem[]> = {
  'pt-BR': [
    ...
    },
    {
      ...
      imageUrl: wellbeingImage?.imageUrl || '',
      imageHint: wellbeingImage?.imageHint || '',
    },
    {
      ...
      imageUrl: themesImage?.imageUrl || '',
      imageHint: themesImage?.imageHint || '',
    },
  ],
  'en-US': [
    {
