  ...
  const handleUpgradeClick = async (planId: 'plus' | 'pro') => {
    ...
    if (!priceId || !priceId.startsWith('price_')) {
      toast({
        ...
        description: t('Stripe configuration error for plan', locale, { plan: planId }),
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: t('Error', locale),
        description: error.message || t('Could not initiate payment process', locale),
        variant: 'destructive',
      });
    } finally {
                ...
                <CardFooter className="flex flex-col items-start gap-4">
                  {!isConfigured && plan.id !== 'free' && (
                     <Alert variant="destructive" className="text-xs">
                        ...
                        <AlertTitle>{t('Configuration needed', locale)}</AlertTitle>
                        <AlertDescription>
                          {t('Stripe Price ID is missing', locale)}
                        </AlertDescription>
                      </Alert>
                  )}
                  {plan.id === 'free' ? (
