...
const getEventSchema = (locale: Locale) => z.object({
  title: z.string().min(1, t('Title is required', locale)),
  date: z.date({
    required_error: t('Date is required', locale),
  }),
  ...
  guests: z.array(z.object({
    email: z.string().email(t('Please enter a valid email.', locale)),
  })).optional(),
});
...
export function AddEventDialog({ isOpen, onOpenChange, onSaveEvent, selectedDate, event }: AddEventDialogProps) {
  ...
  const eventSchema = getEventSchema(locale);
  ...
  // Rerender validation on locale change
  useEffect(() => {
    form.trigger();
  }, [locale, form]);
