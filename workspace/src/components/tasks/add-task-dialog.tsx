...
import type { Task, Locale } from '@/lib/types';
import { AppContext } from '@/context/app-provider';
import { t } from '@/lib/translations';

const getTaskSchema = (locale: Locale) => z.object({
  title: z.string().min(1, t('Title is required', locale)),
  priority: z.enum(['low', 'medium', 'high']),
  dueDate: z.date().optional(),
  project: z.string().optional(),
  subtasks: z.array(z.object({
    title: z.string().min(1, t('Subtask title is required', locale)),
  })).optional(),
});
...
export function AddTaskDialog({ isOpen, onOpenChange, onSaveTask, task }: AddTaskDialogProps) {
  ...
  const taskSchema = getTaskSchema(locale);
